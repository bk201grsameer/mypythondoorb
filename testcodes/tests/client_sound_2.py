...
def audio_thread(socket_connection):
    p = pyaudio.PyAudio()
    stream = p.open(format=FORMAT, channels=CHANNELS, rate=RATE, INPUT=INPUT)
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            frame_matrix = np.reshape(
                frame_matrix, (IMAGE_HEIGHT, IMAGE_WIDTH, COLOR_PIXEL)
            )
